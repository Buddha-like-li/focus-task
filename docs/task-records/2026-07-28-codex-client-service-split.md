...
- **P0 - 旧本地数据不迁移**：按本次边界，客户端不会读取、备份、迁移或删除旧
  ...
  为本次拆分打 tag 或发布。
- 服务镜像启动、导入卷 bootstrap 防护、任务持久化及客户端启动冒烟已由独立
  服务任务和集成人验收：本机 `.7` 服务健康，使用 r13 导入卷且保留 3 用户、
  18 任务和 61 快照。正式服务器部署仍受服务端 P1 mounted-secret-file 改造
  约束，客户端 updater/tag Release 仍受签名限制约束。
